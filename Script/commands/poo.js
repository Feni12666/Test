...
module.exports.run = async function ({ event, api, args, client }) {
 ...
 if (!mention) return api.sendMessage("tag 1 person bitch", event.threadID, event.messageID);
 else {
 var one = mention;
 return makeImage({ one }).then(path => api.sendMessage({ body: event.mentions[mention].replace("@", "") + " Ew, I stepped in a shit", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
 }
}